...
export module beyond.core:math.quat;
...
template <typename T> struct TQuat {
  ...
  [[nodiscard]] friend constexpr auto operator*(const TQuat& lhs,
                                                const TQuat& rhs) noexcept
      -> TQuat
  {
    return TQuat(lhs.w * rhs.x - lhs.x * rhs.w + lhs.y * rhs.z - lhs.z * rhs.y,
