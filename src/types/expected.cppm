...
public:
  ...
  [[nodiscard]] constexpr E& value() & { return m_val; }
  [[nodiscard]] constexpr E&& value() && { return std::move(m_val); }
...
template <class T, class E,
          bool = is_void_or<T, std::conjunction<
...
template <class T, class E,
          std::enable_if_t<
              (std::is_void<T>::value || std::is_move_constructible_v<T>) &&
              std::is_swappable_v<T> && std::is_move_constructible_v<E> &&
