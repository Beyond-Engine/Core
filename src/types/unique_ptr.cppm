...
export module beyond.core:unique_ptr;
...
public:
  ...
  constexpr ~UniquePtr() { delete ptr_; }
  ...
  /// owned.
  [[nodiscard]] constexpr auto get() const noexcept -> pointer { return ptr_; }
  ...
  constexpr void swap(UniquePtr rhs) noexcept { std::swap(ptr_, rhs.ptr_); }

  /// @brief Equality comparison between an UniquePtr and nullptr
  constexpr friend auto operator==(const UniquePtr& lhs, std::nullptr_t) -> bool
  { return lhs.ptr_ == nullptr; }

  /// @brief Threeway comparison between an UniquePtr and nullptr
  constexpr friend auto operator<=>(const UniquePtr& lhs, std::nullptr_t)
      -> std::strong_ordering
  { return lhs.ptr_ <=> nullptr; }
...
template <typename T>
constexpr auto swap(UniquePtr<T>& lhs, UniquePtr<T>& rhs) noexcept
{ lhs.swap(rhs); }
